...
<?php if (have_posts()) : ?>
			<div class="comment-count">
				<h2 class="page-title"><?php _e('Search Results for:', 'la') ?> <span id="search-terms"><?php echo wp_specialchars(stripslashes($_GET['s']), true); ?></span></h2>
			</div>
...
<?php else : ?>
			...
			<div id="post-0" class="post noresults">
				<h2 class="entry-title"><?php _e('Nothing Found', 'la') ?></h2>
				<div class="entry-content">
					<p><?php _e('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'la') ?></p>
				</div>
				<form id="noresults-searchform" method="get" action="<?php bloginfo('home') ?>">
					<div>
						...
						<input id="noresults-searchsubmit" name="searchsubmit" type="submit" value="<?php _e('Find', 'la') ?>" />
					</div>
				</form>
			</div><!-- .post -->
