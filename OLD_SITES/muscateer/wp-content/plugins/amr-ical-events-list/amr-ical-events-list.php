...
/*  Copyright 2009  AmR iCal Events List  (email : anmari@anmari.com)
    ...
    the Free Soft<br />ware Foundation; either version 2 of the License, or
...
function amr_list_properties($icals) {  /* List the calendar properties if requested in options  */
	...
	foreach ($icals as $i => $p)	{ /* go through the options list and list the properties */
		...
		{	
			...
			{	if (!($prevcol === '')) { 
					$cprop .= AMR_NL.AMR_TB.'</ul></div> <!-- end of amrcol -->';
					}  /* end prev column */
				$cprop .= AMR_NL.AMR_TB.'<ul class="amrcol amrcol'.$col.'">';  /* start next column */
				$prevcol = $col;
			}			
...
//				}
			...
			$html .= AMR_NL.''
				.$cprop.AMR_NL.AMR_TB.'</ul> <!-- end of amrcol -->'.AMR_NL
				.''.AMR_NL;  		
			}
	}
	return ($html);
...
/* If there is a event url, use that as href, else use icsurl, use description as title */
  ...
  if (!empty($e_desc)) {
	...
	else $e_desc =  __('Click to view event description', 'amr-ical-events-list');
	...
	if (is_object($content)) {
		...
		switch ($k){
			...
			case 'URL': /* assume valid URL, should not need to validate here, then format it as such */
					return( '<a href="'.$content.'">'.__('Get more info...', 'amr-ical-events-list').'</a>');
...
//		$html = AMR_NL.'<divead><div>';
//		for ($i = 1; $i <= $no_cols; $i++) { 			/* generate the heading code if requested */
//			$html .= AMR_NL.'<div class="amrcol'.$i.'">'.$amr_options[$amr_listtype]['heading'][$i];
//			$html .= '</div>';	
//		}
//		$html .= AMR_NL.'</div></divead>';
//		$html .= AMR_NL.'<div><div>'.AMR_NL
//			.'<div colspan="'.$no_cols.'" style="font-size:x-small; font-weight:lighter;" >';
//			if (function_exists('amr_semi_paginate')) $html .= amr_semi_paginate();
//			else $html .= amr_ngiyabonga();
//		if (!($amrW)) {$html .= amr_show_refresh_option ();}
//		$html .= '</div>'.AMR_NL.'</div></div>';
//
...
//		if ((!is_array($events)) and (count($events) > 0 )) return ('');
		...
		foreach ($events as $i => $e) { /* for each event, loop through the properties and see if we should display */
			...
			foreach ($order as $k => $kv) { /* ie for one event, check how to order the bits */
				...
				if ((isset ($v))  && (!empty($v)))
				{
					...
					if ($col > $prevcol) { /* if new column, then new cell , */
						...
						//	$colcount = $colcount +1;
						//	$eprop .= AMR_NL.'/*<div>&nbsp;</div>*/';
						//}

						$eprop .= AMR_NL.'<div class="amrcol'.$col.' ';
			...
			{	/* then finish off the event or row, save till we know whether to do group change heading first */
				 $eprop = AMR_NL.'<div'.($alt ? ' class="alt':' class="').$classes.'"> '

					.$eprop.AMR_NL.'</ul></div>'.AMR_NL.'</div>';
				...
				{	foreach ($g as $gi=>$v) {	
						...
						if (!($new[$gi] == $old[$gi])) {  /* if there is a grouping change then write the heading for the group */
							...
							$change .= 	'</div></li><li id='.$id
							.' class="group '.$gi. '" ><h3>'  .$grouping. '</h3><div class="inside">';
							$old[$gi] = $new[$gi];							
						}
					} 					
				}				
...
/* amr here is the main calling code  *** */	
		...
		if (isset($calprophtml) and (!(empty($calprophtml))) and (!($calprophtml === ''))) {
			$calprophtml  = '<div id="'.$amrW.'calprop'.$icalno.'" class="vcalendar">'.$calprophtml.'</div>'.AMR_NL;
		}  	
		...
		//added extra to get rid of extra first li 
		else {
				$thecal = $calprophtml
				.AMR_NL.'<ul id="'.$amrW.'compprop'.$icalno.'"><li class="no-show"><h3></h3><div class="inside">'				
